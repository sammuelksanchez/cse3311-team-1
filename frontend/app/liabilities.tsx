...
import { getLiabilityItems, saveLiabilityItem, deleteLiabilityItem } from '@/services/api';
...
export default function LiabilitiesPage() {
  ...
  const handleAdd = async () => {
    ...
    Keyboard.dismiss();
    try {
      const result = await saveLiabilityItem({ name: newName, amount: newAmount, month });
  ...
  const handleDelete = async (itemID: string) => {
    Alert.alert('Delete', 'Remove this liability?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await deleteLiabilityItem(itemID);
  ...
  const handleEditAmount = async (item: Item, newAmt: string) => {
    ...
    try {
      await saveLiabilityItem({ itemID: item.itemID, name: item.name, amount: newAmt, month });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.title}>{month} Liabilities</Text>
